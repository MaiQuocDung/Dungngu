...
<html>
<head>
	<title>QD Shoes | Trang chủ</title>
	<meta charset="utf-8">
		...
		<div class="footer">
			<div class="grid wide">
							...
							if($ok == 2)
					{
						...
						{
									$prodId = $row['id'];
									$qty = isset($_SESSION['cart'][$prodId]) ? intval($_SESSION['cart'][$prodId]) : 0;
									$unitPrice = floatval($row['giasp']);
									$lineTotal = $unitPrice * $qty;
									echo "<div>";
									echo "<h5>".htmlspecialchars($row['tensp'])."</h5>";
									echo "Giá (1 sp): " . number_format($unitPrice,0,',','.') . " đ<br />";
									echo "<p align='right'>Số lượng: <input type='text' name='qty[$prodId]' size='5' value='". $qty ."'> - ";
									echo "<a href='../pages/delcart.php?productid=$prodId'>Xóa</a></p>";
									echo "<p align='right'>Giá tiền cho món hàng: " . number_format($lineTotal,0,',','.') . " đ</p>";
									echo "<hr></div>";
									$total += $lineTotal;    
						}
